<template>
  <div class="detail-facility-wrap">
    ...
    <div class="detail-facility-body" v-if="datas.length >= 1">
      <ul class="clearfix">
    ...
    <div class="detail-facility-body detail-zwsj" v-if="datas.length < 1">
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取服务区详情
      getGIS_FWQDetail (_id) {
        api.getFwqDetail({id: _id}).then(res => {
          this.tableData = res.data
        })
      },
      // 获取出入口详情
      getGIS_CRKDetail (_id) {
        api.getCrkDetail({id: _id}).then(res => {
          this.tableData = res.data
        })
      },
      // 列表点击事件
